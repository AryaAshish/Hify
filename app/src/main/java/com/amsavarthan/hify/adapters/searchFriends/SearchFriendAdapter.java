...
public class SearchFriendAdapter extends RecyclerView.Adapter<SearchFriendAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        checkUserAlreadyFriend(position);

    }

    private void checkIfReqSent(final int pos) {
        ...
        FirebaseFirestore.getInstance()
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                if (documentSnapshot.exists()) {
                    holderr.exist_icon.setVisibility(View.GONE);
                    holderr.friend_icon.setVisibility(View.VISIBLE);
                } else {
                    FirebaseFirestore.getInstance()
                            .collection("Users")
                            .document(usersList.get(pos).userId)
                        ...
                        public void onSuccess(DocumentSnapshot documentSnapshot) {
                            if (documentSnapshot.exists()) {
                                holderr.friend_icon.setVisibility(View.GONE);
                                holderr.exist_icon.setVisibility(View.VISIBLE);
                            } else {
                                holderr.exist_icon.setVisibility(View.GONE);
                                holderr.friend_icon.setVisibility(View.GONE);
                            }
    ...
    private void checkUserAlreadyFriend(int pos) {
        FirebaseFirestore.getInstance().collection("Users").document(usersList.get(pos).userId)
                .collection("Friend_Requests").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                .get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                if (documentSnapshot.exists()) {
                    if (documentSnapshot.getBoolean("accepted")) {
                        holderr.friend_icon.setVisibility(View.VISIBLE);
                        holderr.exist_icon.setVisibility(View.GONE);
                    } else {
                        holderr.friend_icon.setVisibility(View.GONE);
    ...
    private void removeUserandRestoreItem(final int position, final Friends deletedItem) {
        FirebaseFirestore.getInstance().collection("Users").document(usersList.get(position).userId)
                .collection("Friend_Requests").document(deletedItem.userId).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
    ...
    private void executeFriendReq(final int position, final int deletedIndex, final Friends deletedItem) {
        ...
        FirebaseFirestore.getInstance().collection("Users").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                ...
                FirebaseFirestore.getInstance().collection("Users").document(deletedItem.userId)
                        .collection("Friend_Requests")
